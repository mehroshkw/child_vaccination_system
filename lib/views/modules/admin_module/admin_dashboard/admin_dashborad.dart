...
import 'package:child_vaccination_system/views/reusable_widgets/const_function.dart';
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text("Status: ${hospitalData['status']}",
...
class AppointmentTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> appointmentsStream = FirebaseFirestore.instance.collection('appointments').snapshots();
    ...
    return Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                Future<void> generatePdfData(List<DocumentSnapshot> appointments) async {
                  final pdf = pw.Document();

                  for (var appointment in appointments) {
                    Map<String, dynamic> appointmentData = appointment.data() as Map<String, dynamic>;

                    // Extract the required fields from the appointmentData map
                    ...
                    pdf.addPage(pw.Page(
                        ...
                        build: (pw.Context context) => pw.Row(children: [
                              pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                                pw.Text(hospitalName,
                                    style: pw.TextStyle(
                                        fontSize: 28,
                                        fontWeight: pw.FontWeight.bold)),
                                pw.Padding(
                                  padding: const pw.EdgeInsets.only(top: 5.0),
                                  child: pw.Text("appointment Status: $status",
                                      style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                                ),
                                pw.Padding(
                                ...
                                pw.Text("Phone: $phone",
                                    style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                                pw.Text("Patient CNIC: $patientCNIC",
                                    style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                                pw.Text("VaccineName: $vaccination",
                                    style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                                pw.Text("Date: $appointmentDate",
                                    style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                                pw.Text("Booked by: $email",
                                    style: const pw.TextStyle(color: PdfColor.fromInt(0xffCB212A), fontSize: 24)),
                              ])
                            ])));
                  }
                  ...
                  // var path = "${output.path}/appointments.pdf";
                  // final file = File(path);
                  // await file.writeAsBytes(await pdf.save());

                  final file = File('/storage/emulated/0/Download/appointments.pdf');
                  await file.writeAsBytes(await pdf.save());

                  showToast('PDF saved at ${file.path}');
                  // return file.path;
                  // return pdf.save();
                }
                ...
                if (appointments.isEmpty) {
                  ...
                  return const Center(
                      child: Text('No Appointments yet', style: TextStyle(color: Colors.black, fontSize: 16)));
                }
                return Column(children: [
                  ElevatedButton(
                      onPressed: () {
                        generatePdfData(appointments).then((pdfData) {});
                      },
